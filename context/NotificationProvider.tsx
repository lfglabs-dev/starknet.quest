...
export const NotificationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const showNotification = (message: string, type: NotificationProps['type']) => {
    setNotification({ message, type, open: true, onClose: () => hideNotification(), autoHideDuration: 6000 });
  };

  const hideNotification = () => {
    setNotification((prevNotification) => ({ ...prevNotification, open: false }));
  };
  ...
  return (
    <NotificationContext.Provider value={{ showNotification, hideNotification }}>
      ...
      >
        <NotificationContentRoot className={notification.type} aria-describedby="client-snackbar" message={
          <span id="client-snackbar" className="text-white flex gap-2">
            {renderIcon()}
            {notification.message}
          </span>
        } action={[
          <IconButtonRoot key="close" aria-label="Close" color="inherit" onClick={notification.onClose}>
            <CloseIcon />
          </IconButtonRoot>,
        ]} />
      </Snackbar>
